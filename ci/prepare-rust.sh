...
STAMP="$SOURCE/switch/rust/sysroot/.rustc-version"

# Nightly is a rolling toolchain. Reusing a sysroot generated by a different
# nightly leaves incompatible compiler_builtins metadata and makes Meson's
# Rust sanity check fail (this is especially easy to hit on the x86/QEMU
# runner when the checkout contains ignored build outputs).
RUST_VERSION=$(rustc +nightly --version)

if compgen -G "$SYSROOT/libstd-*.rlib" >/dev/null 2>&1 &&
    [[ -f "$STAMP" ]] && grep -Fqx "$RUST_VERSION" "$STAMP"; then
    echo "Using existing Rust sysroot: $SYSROOT"
    exit 0
fi

echo "Rebuilding Rust sysroot for $RUST_VERSION"
